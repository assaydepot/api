...
facets[available_provider_names][]=Assay+Depot
{% endhighlight %}
...
Any number of options can be combined in a single query. For instance, the following (contrived) example, lists the first page of services that match the string "fluid" and are associated with the species Human. It also sets the page size to 2.

{% highlight bash %}
https://www.assaydepot.com/api/wares.json?facets[species][]=Human&q=fluid&page=1&per_page=2
{% endhighlight %}
